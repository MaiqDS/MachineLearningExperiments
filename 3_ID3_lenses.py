...
def createDataset():
    with open("machinelearning\Ch03\lenses.txt", "rb") as f:
        # 读文件时读入的是字节，为对他们进行处理，应先使用decode()方法转化为str再操作。
        dataset = [line.decode().strip().split('\t') for line in f.readlines()]
...
if __name__ == '__main__':
    ...
    x = input()
    xlist = x.split(",")
    print(classify(myTree, featureLabels, xlist))
